...
then
  ...
  docker compose exec -u root -e CI=true zammad-nginx sh -c "curl -fsSL https://deb.nodesource.com/setup_20.x | bash - && DEBIAN_FRONTEND=noninteractive apt-get -q -y install nodejs && npm -g install yarn pnpm"
  docker compose exec -u root -e CI=true zammad-nginx sh -c "DEBIAN_FRONTEND=noninteractive apt-get -q -y install libgtk2.0-0 libgtk-3-0 libgbm-dev libnotify-dev libgconf-2-4 libnss3 libxss1 libasound2 libxtst6 xauth xvfb"
fi
