...
публици](#audience-assumptions). Управо из разлога зато што је један од
...
можете прећи на одељак са неким [примерима](#examples) садржаја.
...
- Користите кратке и јасне реченице, дајући предност информацијама у односу
  на сложеност.
- Користите реченичну капитализацију за наслове страна и поднаслове. Великим
  словом пишите прву реч и прво слово сваке властите именице (нпр. Zammad,
  Elasticsearch, Docker, GitLab) и акронима (SLA, SSO, API, JSON, итд.).
...
- Користите назначавање изворног кода да бисте нагласили секције о
  програмском језику.
- Користите [оквире са информацијама, саветима и упозорењима](#custom-boxes)
  по потреби.
...
| Поља, елементи интерфејса, менији    | **Назив**                     | `**Назив**`                     |
| Навигационе путање                   | _Подешавања > Канали > Имејл_ | `_Подешавања > Канали > Имејл_` |
| Путање датотека и директоријума      | `/opt/zammad`                 | `` `/opt/zammad` ``.            |
| Примери вредности за подешавања      | `<your-fqdn>`                 | `` `<your-fqdn>` ``             |
| Примери вредности за API захтеве     | `{ticket id}`                 | `` `{ticket id}` ``             |
| Конзолне вредности, портови, кључеви | `9200`                        | `` `9200` ``   `                |
...
#### Слике специфичне за тему

Да бисте ограничили одређене слике на једну тему, можете доделити CSS класу
`.dark-only` или `.light-only` одговарајућој слици:

```md
![Само за тамну изглед](/assets/logo-flat-dark.svg){.dark-only}
![Само за светли изглед](/assets/logo-flat-light.svg){.light-only}
```

![Само за тамни изглед](/assets/logo-flat-dark.svg){.dark-only width=240}
![Само за светли изглед](/assets/logo-flat-light.svg){.light-only width=240}
