...
By default, a backup is created at each start of the stack as well as at 3 o'clock each night. The backup is stored in
the volume of the **zammad-backup** container under `/var/tmp/zammad`.
...
5. Start the stack. The restore process is triggered if the `restore` directory is detected and the backup files are in
   place.
