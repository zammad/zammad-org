...
If for any reason you can't complete this tutorial, simply turn off
...
[webserver configuration guide](/en/tutorials/webserver-config.md) before continuing.
...
reach the *domain controller* (Active Directory server).
...
Apache needs a Kerberos *keytab* (key table) to manage its shared
...
service account *after enabling 256-bit AES encryption?*

And did you make sure to register the SPN (with `ktpass`) and generate
your keytab (with `ktutil`) *after changing your password?*
