...
as your recent search queries. To search, simply type a term. The search then displays matching items for which you
have at least view or read permissions, grouped by type like users and tickets. Selecting one of those results opens the
item as tab in the taskbar. The number of results per type is limited: as soon as your search matches more than two
types, each group shows up to five results. Use the detailed search to get the full result set.
...
**Ticket**, the search also returns tickets where the owner or customer matches the search term. Knowledge base
answers are available as an object type of their own, with title, visibility and update date as result columns. See
[Knowledge base](/en/documentation/use/guides/knowledge-base#search) for searching within the knowledge base.
